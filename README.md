...
This is a complete, back-end Hotel Management System built with Spring Boot. It provides a RESTful API for both guests to search, book, and pay for hotel rooms, and for hotel managers to manage their properties, inventory, and pricing.
...
Role-Based Access Control: The system defines two distinct roles: GUEST and HOTEL_MANAGER.
...
2. Guest Portal (/hotels, /bookings, /users)
Search Hotels: Guests can search for available hotels by city, check-in/check-out dates, and number of rooms.
...
Hotel Management: Hotel managers can perform full CRUD operations on their hotels. Hotels are created as inactive and must be activated via an endpoint to begin populating inventory.
...
The system uses the Strategy Pattern to calculate room prices dynamically at the time of booking. The PricingService chains multiple pricing strategies together:
...
Inventory Generation: When a hotel is activated or a new room is added, the system automatically generates inventory records for that room for one year in advance.
...
The booking process is a stateful workflow managed by BookingStatus:
...
git clone https://github.com/rohitrathore45/hotel_management_system.git
cd hotel_management_system
2. Configure the Database
Open your PostgreSQL admin tool (e.g., psql or PGAdmin).
...
The project uses application.properties for configuration. This file is included in the .gitignore, so you will need to create it.
...
spring.datasource.url=jdbc:postgresql://localhost:5432/hotel_system
...
You can run the application using the built-in Maven wrapper.
